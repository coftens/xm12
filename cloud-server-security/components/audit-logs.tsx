...
const categoryColors: Record<string, string> = {
  firewall: "bg-primary/10 text-primary border-primary/20",
  access: "bg-info/10 text-info border-info/20",
  security: "bg-danger/10 text-danger border-danger/20",
  keys: "bg-warning/10 text-warning border-warning/20",
}
          ...
          <div className="flex flex-col divide-y divide-border">
            {paginatedLogs.map((log) => {
              ...
              return (
                <div
                  key={log.id}
                  className="flex flex-col hover:bg-secondary/20 transition-colors cursor-pointer"
                  onClick={() => setExpandedLog(isExpanded ? null : log.id)}
                >
                  <div className="flex items-center gap-4 px-5 py-3.5">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${categoryColors[log.category]?.split(" ")[0] || "bg-primary/10"}`}>
                      <Icon className={`w-4 h-4 ${categoryColors[log.category]?.split(" ")[1] || "text-primary"}`} />
                    </div>
                    ...
                    <Badge className={`text-[10px] border-0 shrink-0 ${resultColors[log.result]}`}>
                    ...
                    <div className="px-5 pb-3.5 pl-17">
                      <div className="p-3 rounded-lg bg-secondary/30 border border-border">
                        <p className="text-xs text-muted-foreground">
                          <span className="text-foreground font-medium">详情：</span> {log.details}
                        </p>
                        <p className="text-xs text-muted-foreground mt-1">
                          <span className="text-foreground font-medium">日志 ID：</span>

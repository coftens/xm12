...
import { Users, KeyRound, Lock, Fingerprint } from 'lucide-react'

// ─── Access Control ───────────────────────────────────────────────────────────
const roles = [
    { name: '超级管理员', users: 2, permissions: 48, color: 'bg-red-500/10 text-red-500' },
    { name: '运维工程师', users: 5, permissions: 32, color: 'bg-amber-500/10 text-amber-500' },
    { name: '开发人员', users: 12, permissions: 18, color: 'bg-sky-500/10 text-sky-500' },
                ...
                {roles.map(role => (
                    <Card key={role.name}>
                        <CardContent className="p-4">
                            ...
                            <div className="flex items-baseline gap-2"><span className="text-2xl font-bold">{role.users}</span><span className="text-xs text-muted-foreground">个用户</span></div>
            ...
            <Card>
                <CardContent className="p-0">
                    <div className="px-5 py-3 border-b border-border flex items-center justify-between">
                        ...
                        <Button size="sm" className="text-xs">添加用户</Button>
                    </div>
                            ...
                            <div key={user.email} className="flex items-center gap-4 px-5 py-3 hover:bg-secondary/20 transition-colors">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div className={`w-1.5 h-1.5 rounded-full ${user.status === 'online' ? 'bg-emerald-500' : 'bg-muted-foreground/40'}`} />
                                    </div>
                                    <span className="text-xs text-muted-foreground">{user.email}</span>
                                </div>
                                <Badge variant="outline" className="text-[10px] border-border text-muted-foreground">{user.role}</Badge>
                                <div className="flex items-center gap-1.5">
                                    <Fingerprint className={`w-3.5 h-3.5 ${user.mfa ? 'text-emerald-500' : 'text-muted-foreground/40'}`} />
...
const keyStatusColors = { active: 'bg-emerald-500/10 text-emerald-500', expiring: 'bg-amber-500/10 text-amber-500', expired: 'bg-red-500/10 text-red-500' }
const keyStatusLabels = { active: '有效', expiring: '即将过期', expired: '已过期' }

export function KeyManagementPage() {
    return (
        <div className="flex flex-col gap-6 p-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[{ label: '有效密钥', status: 'active', color: 'text-emerald-500', bg: 'bg-emerald-500/10' }, { label: '即将过期', status: 'expiring', color: 'text-amber-500', bg: 'bg-amber-500/10' }, { label: '已过期', status: 'expired', color: 'text-red-500', bg: 'bg-red-500/10' }].map(s => (
                    <Card key={s.label}>
                        <CardContent className="p-4 flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-xl ${s.bg} flex items-center justify-center`}><KeyRound className={`w-5 h-5 ${s.color}`} /></div>
                            <div><p className="text-xs text-muted-foreground">{s.label}</p><p className="text-xl font-bold">{keys.filter(k => k.status === s.status).length}</p></div>
                        </CardContent>
                    </Card>
                ))}
            </div>
            <Card>
                <CardContent className="p-0">
                    <div className="px-5 py-3 border-b border-border flex items-center justify-between">
                        <span className="text-sm font-medium">密钥与证书</span>
                        <Button size="sm" className="text-xs">创建密钥</Button>
                    </div>
                    <div className="divide-y divide-border">
                        {keys.map(key => (
                            <div key={key.name} className="flex items-center gap-4 px-5 py-3.5 hover:bg-secondary/20 transition-colors">
                                <div className="w-8 h-8 rounded-lg bg-secondary flex items-center justify-center shrink-0"><Lock className="w-4 h-4 text-muted-foreground" /></div>
                                <div className="flex-1 min-w-0">
                                    <span className="text-sm font-medium font-mono">{key.name}</span>
                                    <p className="text-xs text-muted-foreground">{key.type}</p>
                                </div>
                                <Badge className={`text-[10px] border-0 ${keyStatusColors[key.status]}`}>{keyStatusLabels[key.status]}</Badge>
                                ...
                                <Button variant="outline" size="sm" className="text-xs shrink-0">轮换</Button>
                            </div>
                        ))}
                    </div>
                </CardContent>
...
export function SecuritySettingsPage() {
    const [states, setStates] = useState(Object.fromEntries(settingsData.map(s => [s.id, s.enabled])))
    ...
    return (
        <div className="flex flex-col gap-6 p-6">
            {categories.map(cat => (
                <Card key={cat}>
                    <CardContent className="p-0">
                        <div className="px-5 py-3 border-b border-border"><span className="text-sm font-medium">{cat}设置</span></div>
                        <div className="divide-y divide-border">
                            {settingsData.filter(s => s.category === cat).map(setting => (
                                <div key={setting.id} className="flex items-center justify-between px-5 py-4 hover:bg-secondary/20 transition-colors">
                                    ...
                                    <Switch checked={states[setting.id]} onCheckedChange={v => setStates(prev => ({ ...prev, [setting.id]: v }))} />
                                </div>
                            ))}
                        </div>
                    </CardContent>

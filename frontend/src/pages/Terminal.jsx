...
// ... (keep SessionTimer, StatusBar, SessionTabs, TerminalHeader, EmptyTerminal, ServerSidebar)
...
function TerminalView({ server, session, isActive, onStatusChange }) {
  ...
  useEffect(() => {
    ...
    ws.onopen = () => {
      ...
      // Send initial path if present and not sent
      if (session.initialPath && !initialPathSent.current) {
        term.writeln(`\x1b[33m跳转到: ${session.initialPath}\x1b[0m`)
        ws.send(JSON.stringify({ type: 'input', data: `cd "${session.initialPath}"\r` }))
...
export default function SSHTerminalPage() {
  ...
  const connectToServer = useCallback((server, initialPath = null) => {
    const existing = sessions.find(s => s.serverId === server.id && s.status === 'connected')
    if (existing) {
      // If requesting a new path on an existing connection, maybe we should just send a cd command?
      // But existing structure doesn't easily allow "injecting" a command into an running session from here without context.
      // For now, if we are Auto-connecting (initial load), we probably don't have an existing session.
      // If user clicks "Open Terminal" again, it might open a new tab or reuse.
      // Let's create a new session if initialPath is provided, to ensure we are in the right dir?
      // OR just focus existing.
      // Given the UI allows multiple sessions, maybe best to open a new one if path is different?
      // Let's stick to simple logic: focus existing if no path, new session if path?
      // Actually, simplifying: "Open Terminal Here" implies "Give me a terminal at this path".
      // If I reuse an existing one, I need to send `cd`.
      // For now, let's create a new session if initialPath is set, or just reusing `existing` but `setActiveSessionId` is safer.
      // But if we want to support "Open specific folder", a new session is cleaner.
    ...
    const newSession = {
      ...
      initialPath, // Store initial path
    }
  ...
  useEffect(() => {
    if (servers.length > 0 && !autoConnectDone.current) {
      const serverId = searchParams.get('server')
      const path = searchParams.get('path')

      if (serverId) {
        const targetServer = servers.find(s => String(s.id) === String(serverId) || s.id === Number(serverId))
        if (targetServer) {
          connectToServer(targetServer, path)
        }
      }
      autoConnectDone.current = true
    }
  }, [servers, searchParams, connectToServer])
      ...
      <div className="flex h-[calc(100vh-80px)] w-full overflow-hidden rounded-lg border border-border bg-background">
        ...
        {!isFullscreen && (
          <ServerSidebar
            ...
            onServerSelect={s => connectToServer(s)}
